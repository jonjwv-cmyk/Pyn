...
export interface FlowRow {
  ...
  stat_manual: number;
  /** Подстатус справочника (B1), пусто если лист. */
  stat_sub?: string;
  /** STAT NOTE — свободный комментарий к статусу (не справочник). */
  stat_note?: string;
  /**
   * JSON-массив ручных статусов из Плана/Отчёта (A1):
   * [{stat,sub,note,src,at}, …]. В UI: авто — 1-я строка; стек — ниже, жирным.
   */
  stat_stack?: string;
  time_at: string;
