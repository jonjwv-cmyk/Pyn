...
export interface Warehouse {
  ...
  /**
   * Кластер «Технология» — справочный признак склада (юзер 2026-07-05).
   * К плану/графику отношения НЕ имеет, нигде в расчётах не учитывается —
   * пригодится для сайта (как справочные роли потока).
   */
  tech_cluster: 0 | 1;
  ...
  /** Кто пометил «удалён»: 'auto' (нет в выгрузке) / 'manual' (вручную). null если не удалён. */
  removal_kind?: 'auto' | 'manual' | null;
  /** Месяц удаления YYYY-MM — для окна «Склады удалены» в графике. null если не удалён. */
