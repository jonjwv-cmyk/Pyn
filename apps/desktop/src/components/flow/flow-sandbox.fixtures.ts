...
/**
 * Значения выпадашки STAT: ручные (ставятся напрямую) + `мало` (выбор при ПРОЙДЕННОЙ
 * норме → конфликт → карточка ВГХ для правки MIN QTY). `мет_ок` НЕ в списке — он чисто
 * авто и только для склада-отправителя 9002 (норма прошла); масловоз/прекурсор — тоже
 * авто по номенклатуре. Снять расчётный = поставить ручной ИЛИ карточка (в пусто).
 */
