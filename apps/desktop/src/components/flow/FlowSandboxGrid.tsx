...
export function FlowSandboxGrid(): JSX.Element {
  ...
  const getCellContentRaw = useCallback(
    (cellPos: Item): GridCell => {
      ...
      if (spec.kind === 'window') {
        // Доставка — окно ДНЕВНОЙ смены (08:30 → конец смены, обед 12:00–12:45).
        // Конец смены зависит от даты этой строки по графику (колонка ГРАФ):
        // ПН-ЧТ 16:30 / ПТ 15:00, −1ч в предпраздничные. Считаем от той же
        // ближайшей даты, что и ГРАФ; нет даты → дефолт 16:30 в самой ячейке.
